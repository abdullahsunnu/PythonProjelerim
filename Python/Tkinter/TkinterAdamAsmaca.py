...
class AdamOyun():
   ...
   _/_\      |
    ...
    def dosyaAc(self):
        try:
            dsy = open(os.getcwd() + os.sep + self.dosyaAdi,  "r", encoding="Utf-8" )
            return dsy
        except:
            return False
    ...
    def dosyaAlma(self):
        ...
        if dsy:  # Dosya açılırsa kelimeleri dosyadan alıp listeye aktaracak
            ...
            showerror("Hata", "Yüklemeye çalıştığınız {} adlı dosya, programın çalışma dizininde bulunamadı !".format(
                self.girisDosya.get()))
            showwarning("Uyarı",
